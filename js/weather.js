...
$(function() {
    ...
    if (Storage.length !== null) {
        ...
        (function theLoop(i) {
            setTimeout(function() {
                if (--i) {
                    savedLocations = localStorage.getItem(
                        'savedLocations[' + (i) + ']') || [];
    ...
    $("#addLocation").click(function() {
        ...
        localStorage.setItem((('savedLocations[' + widgetId +
            ']')), JSON.stringify(oldItems));
        widgetId++;
        /////FIGURED OUT SAVING. Around 10 hours of testing and errors.
        log(("Testing " + (widgetId - 1) + "[0] : " +
            localStorage['savedLocations[' + (widgetId -
                1) + ']']));
        var testing = JSON.parse(localStorage['savedLocations[' +
            (widgetId - 1) + ']']);
    ...
    $(document).on('click', '#btnClose', function() {
        ...
        log("Removoving : 'savedLocations[" + $(this).closest(
            'div[id]').attr('id') + "]'");
        localStorage.removeItem(['savedLocations[' + $(this).closest(
            'div[id]').attr('id') + ']']);
        log("Local Storage Contents : " + localStorage);
...
function addLocation(name, lat, lng, widgetId, divTemp) {
    ...
    $('<div/>', {
        ...
        text: currentLocation === "null" ? "Unknown" : (name.substring(
            0, 22) + "     " + divTemp + String.fromCharCode(
            176) + "F"),
        lat: lat,
        lng: lng,
        name: name
    ...
    log(widgetId);
    $('#' + widgetId).css({
        "background": "url(./img/" + getBackground(icon) +
            ".jpg) no-repeat",
    });
}
    ...
    $.ajax({
        ...
        success: function(result) {
            ...
            if (widgetId > 0) {
                ...
                $("#tempText").text(parseInt(result.currently.temperature) +
                    String.fromCharCode(176) + "F");
                $("#statusText").text(result.currently.summary);
                log(result.daily);
                $("#minTemp").text(parseInt(result.daily.data[0]
                    .temperatureMin) + String.fromCharCode(
                    176) + "F");
                $("#maxTemp").text(parseInt(result.daily.data[0]
                    .temperatureMax) + String.fromCharCode(
                    176) + "F");
                $("#percipChance").text(parseInt(result.daily.data[
                    0].precipProbability * 100) + "%");
                $('#appDiv').css({
                    "background": "url(./img/" +
                        background + ".jpg) no-repeat",
                });
                //Set Weekly data
                $("#monday").text("Monday : " + result.daily.data[
                    0].summary + " " + parseInt(result.daily
                    .data[0].temperatureMax) + String.fromCharCode(
                    176) + "F");
                $("#monday").css({
                    "background": "url(./img/" +
                        getBackground(result.daily.data[
                            0].icon) +
                        ".jpg) no-repeat",
                });
                $("#tuesday").text("Tuesday : " + result.daily.data[
                    1].summary + " " + parseInt(result.daily
                    .data[1].temperatureMax) + String.fromCharCode(
                    176) + "F");
                $("#tuesday").css({
                    "background": "url(./img/" +
                        getBackground(result.daily.data[
                ...
                $("#wenesday").text("Wenesday : " + result.daily
                    .data[2].summary + " " + parseInt(
                        result.daily.data[2].temperatureMax
                    ) + String.fromCharCode(176) + "F");
                $("#wenesday").css({
                    "background": "url(./img/" +
                        getBackground(result.daily.data[
                ...
                $("#thursday").text("Thursday : " + result.daily
                    .data[3].summary + " " + parseInt(
                        result.daily.data[3].temperatureMax
                    ) + String.fromCharCode(176) + "F");
                $("#thursday").css({
                    "background": "url(./img/" +
                        getBackground(result.daily.data[
                ...
                $("#friday").text("Friday : " + result.daily.data[
                    4].summary + " " + parseInt(result.daily
                    .data[4].temperatureMax) + String.fromCharCode(
                    176) + "F");
                $("#friday").css({
                    "background": "url(./img/" +
                        getBackground(result.daily.data[
                ...
                $("#saturday").text("Saturday : " + result.daily
                    .data[5].summary + " " + parseInt(
                        result.daily.data[5].temperatureMax
                    ) + String.fromCharCode(176) + "F");
                $("#saturday").css({
                    "background": "url(./img/" +
                        getBackground(result.daily.data[
                ...
                $("#sunday").text("Sunday : " + result.daily.data[
                    6].summary + " " + parseInt(result.daily
                    .data[6].temperatureMax) + String.fromCharCode(
                    176) + "F");
                $("#sunday").css({
                    "background": "url(./img/" +
                        getBackground(result.daily.data[
